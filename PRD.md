...
## Sprint 3 — Correção de viés na leitura + expansão do reconto

**Motivação:** A criança descobriu que conseguia acertar a interpretação de texto sem ler, "chutando pela alternativa mais longa". Medição confirmou viés severo (correta era a mais longa em 98% e estava no índice 1 em 85%, nunca no índice 3). Além disso, pedido de ampliar o reconto para 100 histórias, com a maioria mais difícil.

- [x] **Correção do viés de comprimento+posição (leitura):** distratores reescritos/equilibrados no gerador `tools/gerar-bancos.js` (frases completas; `equilibrarComprimento` alonga o maior distrator em ~70% das perguntas por PRNG seeded, sem inverter o viés) e posição da correta randomizada na geração; `embaralharPergunta` em `js/games/leitura.js` embaralha as alternativas em runtime remapeando `correta`. Resultado: correta-mais-longa **98%→22,9%**, comprimento médio correta **64→72** vs distrator **32→71** (diferença 0,7 char), posição uniforme `{0:283,1:276,2:262,3:304}`. Banco regenerado (IDs estáveis); escrita/matemática byte-idênticos. Teste de regressão `tests/leitura-vies.test.js`.
- [x] **Expansão do reconto para 100 histórias:** banco passou de 50 → **100** (30 fáceis + 35 médias + 35 difíceis = 70 mais difíceis que o nível anterior, calibradas ~10-13 anos com reviravoltas/dilemas). Geradas por 4 agentes em paralelo via pipeline de fragmentos: `tools/reconto-fragmentos/*.js` + validador `tools/checar-fragmento.js` (auto-detecção ≥4/5 por história) + montador `tools/montar-reconto.js` (valida ids únicos/schema/distribuição/NUL e gera `js/data/reconto-exercicios.js`). Auto-detecção média 4,95/5. Teste de integridade atualizado (100, ≥70 médio+difícil).
- [x] Suíte total: **174 testes verdes**. Cache `?v=8`.
